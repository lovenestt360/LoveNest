...
// ── Paleta emocional por fase — muda o estado de ânimo visual de toda a página
const PHASE_PALETTE: Record<string, {
  accent: string; label: string; quote: string; quoteBg: string; quoteBorder: string;
  dot: string; heroBg: string; heroGlow: string;
}> = {
  menstrual: {
    accent:      "text-rose-500",
    label:       "text-rose-600 dark:text-rose-400",
    quote:       "text-rose-600 dark:text-rose-300",
    ...
    dot:         "bg-rose-400",
    heroBg:      "from-rose-50/30 dark:from-rose-950/20",
    heroGlow:    "rgba(244,63,94,0.12)",
  },
  folicular: {
    accent:      "text-sky-500",
    label:       "text-sky-600 dark:text-sky-400",
    quote:       "text-sky-600 dark:text-sky-300",
    ...
    dot:         "bg-sky-400",
    heroBg:      "from-sky-50/30 dark:from-sky-950/20",
    heroGlow:    "rgba(14,165,233,0.12)",
  },
  ovulacao: {
    accent:      "text-emerald-500",
    label:       "text-emerald-600 dark:text-emerald-400",
    quote:       "text-emerald-600 dark:text-emerald-300",
    ...
    dot:         "bg-emerald-400",
    heroBg:      "from-emerald-50/30 dark:from-emerald-950/20",
    heroGlow:    "rgba(16,185,129,0.12)",
  },
  luteal: {
    accent:      "text-violet-500",
    label:       "text-violet-600 dark:text-violet-400",
    quote:       "text-violet-600 dark:text-violet-300",
    ...
    dot:         "bg-violet-400",
    heroBg:      "from-violet-50/30 dark:from-violet-950/20",
    heroGlow:    "rgba(139,92,246,0.12)",
  },
  sem_dados: {
    accent:      "text-rose-400",
    label:       "text-rose-500 dark:text-rose-400",
    quote:       "text-rose-500 dark:text-rose-300",
    quoteBg:     "bg-gradient-to-br from-rose-50/60 to-rose-100/20 dark:from-rose-950/30 dark:to-rose-900/10",
    quoteBorder: "border-rose-200/50 dark:border-rose-800/30",
    dot:         "bg-rose-300",
    heroBg:      "from-rose-50/20 dark:from-rose-950/10",
    heroGlow:    "rgba(244,63,94,0.08)",
  },
};
...
// Cada secção de sintomas tem identidade própria — inactive já tem fundo suave colorido
const SECTION_STYLES: Record<string, {
  soft: string; solid: string; text: string; border: string;
  inactiveBg: string; inactiveBorder: string; inactiveText: string; glow: string;
  labelColor: string;
}> = {
  "Físicos": {
    soft: "bg-rose-50 dark:bg-rose-950/40", solid: "bg-rose-500",
    text: "text-rose-500", border: "border-rose-300 dark:border-rose-700",
    inactiveBg: "bg-rose-50/60 dark:bg-rose-950/20",
    inactiveBorder: "border-rose-100/80 dark:border-rose-900/40",
    inactiveText: "text-rose-400/70 dark:text-rose-500/50",
    glow: "shadow-[0_4px_18px_rgba(244,63,94,0.28)]",
    labelColor: "text-rose-500 dark:text-rose-400",
  },
  "Emocionais": {
    soft: "bg-violet-50 dark:bg-violet-950/40", solid: "bg-violet-500",
    text: "text-violet-500", border: "border-violet-300 dark:border-violet-700",
    inactiveBg: "bg-violet-50/60 dark:bg-violet-950/20",
    inactiveBorder: "border-violet-100/80 dark:border-violet-900/40",
    inactiveText: "text-violet-400/70 dark:text-violet-500/50",
    glow: "shadow-[0_4px_18px_rgba(139,92,246,0.28)]",
    labelColor: "text-violet-500 dark:text-violet-400",
  },
  "Digestivo": {
    soft: "bg-sky-50 dark:bg-sky-950/40", solid: "bg-sky-500",
    text: "text-sky-500", border: "border-sky-300 dark:border-sky-700",
    inactiveBg: "bg-sky-50/60 dark:bg-sky-950/20",
    inactiveBorder: "border-sky-100/80 dark:border-sky-900/40",
    inactiveText: "text-sky-400/70 dark:text-sky-500/50",
    glow: "shadow-[0_4px_18px_rgba(14,165,233,0.28)]",
    labelColor: "text-sky-500 dark:text-sky-400",
  },
  "Outros": {
    soft: "bg-emerald-50 dark:bg-emerald-950/40", solid: "bg-emerald-500",
    text: "text-emerald-500", border: "border-emerald-300 dark:border-emerald-700",
    inactiveBg: "bg-emerald-50/60 dark:bg-emerald-950/20",
    inactiveBorder: "border-emerald-100/80 dark:border-emerald-900/40",
    inactiveText: "text-emerald-400/70 dark:text-emerald-500/50",
    glow: "shadow-[0_4px_18px_rgba(16,185,129,0.28)]",
    labelColor: "text-emerald-500 dark:text-emerald-400",
  },
};

// Cada métrica tem a sua cor — não mais tudo cinzento
const FEELING_STYLES: Record<string, { text: string; bg: string; border: string; labelColor: string; glow: string }> = {
  rose:    { text: "text-rose-500",    bg: "bg-rose-50 dark:bg-rose-950/40",      border: "border-rose-300 dark:border-rose-700",    labelColor: "text-rose-500 dark:text-rose-400",    glow: "shadow-[0_4px_16px_rgba(244,63,94,0.26)]" },
  orange:  { text: "text-orange-500",  bg: "bg-orange-50 dark:bg-orange-950/40",  border: "border-orange-300 dark:border-orange-700",  labelColor: "text-orange-500 dark:text-orange-400",  glow: "shadow-[0_4px_16px_rgba(249,115,22,0.26)]" },
  fuchsia: { text: "text-fuchsia-500", bg: "bg-fuchsia-50 dark:bg-fuchsia-950/40", border: "border-fuchsia-300 dark:border-fuchsia-700", labelColor: "text-fuchsia-500 dark:text-fuchsia-400", glow: "shadow-[0_4px_16px_rgba(217,70,239,0.26)]" },
  red:     { text: "text-red-500",     bg: "bg-red-50 dark:bg-red-950/40",        border: "border-red-300 dark:border-red-700",        labelColor: "text-pink-500 dark:text-pink-400",      glow: "shadow-[0_4px_16px_rgba(239,68,68,0.26)]" },
  teal:    { text: "text-teal-500",    bg: "bg-teal-50 dark:bg-teal-950/40",      border: "border-teal-300 dark:border-teal-700",      labelColor: "text-teal-500 dark:text-teal-400",      glow: "shadow-[0_4px_16px_rgba(20,184,166,0.26)]" },
  indigo:  { text: "text-indigo-500",  bg: "bg-indigo-50 dark:bg-indigo-950/40",  border: "border-indigo-300 dark:border-indigo-700",  labelColor: "text-indigo-500 dark:text-indigo-400",  glow: "shadow-[0_4px_16px_rgba(99,102,241,0.26)]" },
  pink:    { text: "text-pink-500",    bg: "bg-pink-50 dark:bg-pink-950/40",      border: "border-pink-300 dark:border-pink-700",      labelColor: "text-pink-500 dark:text-pink-400",      glow: "shadow-[0_4px_16px_rgba(236,72,153,0.26)]" },
};
...
const PAIN_OPTIONS: ChipOption[]   = [{ value: 0, label: "Nenhuma", icon: Smile }, { value: 3, label: "Leve", icon: Meh }, { value: 6, label: "Moderada", icon: Frown }, { value: 9, label: "Intensa", icon: AlertTriangle }];
const ENERGY_OPTIONS: ChipOption[] = [{ value: 2, label: "Baixa", icon: BatteryLow }, { value: 5, label: "Normal", icon: Battery }, { value: 9, label: "Alta", icon: Rocket }];
const STRESS_OPTIONS: ChipOption[] = [{ value: 0, label: "Calma", icon: Smile }, { value: 4, label: "Normal", icon: Meh }, { value: 8, label: "Stressada", icon: Flame }];
const LIBIDO_OPTIONS: ChipOption[] = [{ value: 2, label: "Baixo", icon: Snowflake }, { value: 5, label: "Normal", icon: Thermometer }, { value: 9, label: "Alto", icon: Flame }];
const SLEEP_QUALITY_OPTIONS = [{ value: "mau", label: "Mau", icon: Moon }, { value: "ok", label: "Ok", icon: Meh }, { value: "bom", label: "Bom", icon: Smile }];
const DISCHARGE_OPTIONS    = [{ value: "seco", label: "Seco", icon: Sun }, { value: "pegajoso", label: "Pegajoso", icon: Droplet }, { value: "cremoso", label: "Cremoso", icon: Droplets }, { value: "clara_de_ovo", label: "Fértil", icon: Egg }];

function valueToChip(value: number, options: ChipOption[]): number {
  let closest = options[0].value, minDiff = Math.abs(value - closest);
  for (const opt of options) { const d = Math.abs(value - opt.value); if (d < minDiff) { minDiff = d; closest = opt.value; } }
  return closest;
}

// ── Chip numérico com label colorido e glow ao selecionar
function ChipSelector({ label, options, value, onChange, disabled = false, color = "rose" }: {
  label: string; options: ChipOption[]; value: number; onChange: (v: number) => void;
  disabled?: boolean; color?: keyof typeof FEELING_STYLES;
}) {
  const style = FEELING_STYLES[color];
  return (
    <div className="space-y-2.5">
      <p className={cn("text-[11px] font-bold uppercase tracking-widest", style.labelColor)}>{label}</p>
          ...
          return (
            <button key={opt.value} type="button" disabled={disabled} onClick={() => onChange(opt.value)}
              className={cn(
                ...
                active
                  ? cn(style.border, style.bg, style.text, style.glow, "scale-[1.05]")
                  : "border-border/50 text-muted-foreground/60 hover:bg-muted/60 hover:border-border/80 active:scale-95",
                disabled && "opacity-50 cursor-not-allowed",
              )}>
              {Icon && <Icon className={cn("h-3.5 w-3.5 transition-all duration-200", active ? "" : "opacity-50")} strokeWidth={active ? 2 : 1.5} />}
              {opt.label}
            </button>
          );
        })}
...
// ── Chip textual — mesma filosofia
function StringChipSelector({ label, options, value, onChange, disabled = false, color = "rose" }: {
  label: string; options: { value: string; label: string; icon?: LucideIcon }[];
...
// ── Fluxo — dots HTML, escala ao selecionar
function FlowChip({ flowLevel, onChange }: { flowLevel: string; onChange: (v: string) => void }) {
  const flows = [{ value: "light", label: "Leve", dots: 1 }, { value: "medium", label: "Moderado", dots: 2 }, { value: "heavy", label: "Intenso", dots: 3 }];
  return (
    <div className="space-y-2.5">
      <p className="text-[11px] font-bold uppercase tracking-widest text-rose-500 dark:text-rose-400">Fluxo</p>
          ...
          return (
            <button key={f.value} type="button" onClick={() => onChange(f.value)}
              className={cn(
                ...
                active
                  ? "border-rose-300 dark:border-rose-700 bg-rose-50 dark:bg-rose-950/40 text-rose-500 shadow-[0_4px_16px_rgba(244,63,94,0.26)] scale-[1.05]"
                  : "border-rose-100/70 dark:border-rose-900/30 bg-rose-50/40 dark:bg-rose-950/15 text-rose-400/60 hover:scale-[1.02] active:scale-95"
              )}>
              <span className="flex gap-1">
                {Array.from({ length: f.dots }).map((_, i) => (
                  <span key={i} className={cn("w-2 h-2 rounded-full transition-all duration-200", active ? "bg-rose-400" : "bg-rose-200 dark:bg-rose-800/60")} />
                ))}
              </span>
              {f.label}
            </button>
...
// ── Cabeçalho de secção — linha gradiente topo + marcador + título
function SectionHeader({ accentFrom, markerClass, children, right }: {
  accentFrom: string; markerClass: string; children: React.ReactNode; right?: React.ReactNode;
}) {
  return (
    <>
...
// ── Card de quote emocional — a alma do card de fase, com cor da fase
function PhaseQuoteCard({ insight, phase, phaseLabel }: { insight: string; phase: string; phaseLabel: string }) {
  const palette = PHASE_PALETTE[phase] ?? PHASE_PALETTE.sem_dados;
  return (
    <div className={cn("rounded-3xl border px-5 py-5", palette.quoteBg, palette.quoteBorder)}>
      ...
      <div className="flex items-center gap-2 mt-3.5">
        ...
        <p className={cn("text-[10px] font-bold uppercase tracking-[0.18em] opacity-70", palette.quote)}>
          {phaseLabel}
        </p>
      </div>
    </div>
...
export function CycleToday({ data }: { data: CycleData }) {
  ...
  const triggerSavedFeedback = useCallback(() => {
    ...
    setSaved(true); setShowPetals(true);
    setTimeout(() => { setSaved(false); setShowPetals(false); }, 2600);
  }, []);

  const handleStartPeriod = async () => {
    if (!user || !spaceId) return;
    setSaving(true); await ensureProfile();
    await supabase.from("period_entries").insert({ user_id: user.id, couple_space_id: spaceId, start_date: startDate, flow_level: flowLevel, pain_level: 0, pms_level: 0, notes: periodNotes || null });
  ...
  const handleSaveIntimacy = async () => {
    if (!user || !spaceId) return;
    setSavingIntimacy(true); await ensureProfile();
    const existing = intimacyLogs.find(l => l.day_key === intimacyDate) ?? null;
    if (!hadActivityToday) {
      if (existing) await (supabase.from("intimacy_logs" as any).delete().eq("id", existing.id) as any);
    } else {
      const payload = { user_id: user.id, couple_space_id: spaceId, day_key: intimacyDate, protection_method: protectionMethod };
      if (existing) await (supabase.from("intimacy_logs" as any).update(payload).eq("id", existing.id) as any);
      else await (supabase.from("intimacy_logs" as any).insert(payload) as any);
    }
    setSavingIntimacy(false); setShowIntimacyDate(false); setIntimacyDate(today);
    triggerSavedFeedback(); reload();
  };

  const periodDays = openPeriod ? differenceInDays(new Date(), new Date(openPeriod.start_date + "T12:00:00")) + 1 : 0;
  ...
  if (data.isMale && !profile) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center space-y-3 opacity-70">
        ...
        <p className="text-xs text-muted-foreground max-w-[250px]">A tua parceira ainda não configurou o acompanhamento do ciclo menstrual.</p>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="space-y-6">
        ...
        {/* ── Hero de fase com atmosfera da fase ── */}
        {engine ? (
          <div className="glass-card relative overflow-hidden">
            {/* Glow ambiente da fase — muito suave */}
            ...
            <div className="relative p-6 space-y-4">
              <div className="flex items-center gap-5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <h2 className={cn("text-2xl font-bold tracking-tight", palette.accent)}>{engine.phaseLabel}</h2>
              ...
              <div className="flex gap-2 flex-wrap">
                {engine.isInPeriod && (
                  <span className="px-3 py-1 rounded-full text-[11px] font-semibold bg-rose-50 dark:bg-rose-950/40 text-rose-500 border border-rose-200/70 dark:border-rose-800/40">Em período</span>
                )}
              ...
              {engine.insights.slice(1).map((insight, i) => (
                <div key={i} className="rounded-xl border border-rose-200/50 dark:border-rose-800/40 bg-rose-50/60 dark:bg-rose-950/20 px-3 py-2 flex items-start gap-2">
        ...
        {/* ── Quote emocional da fase — card próprio, com cor da fase ── */}
        {engine?.insights[0] && (
          <PhaseQuoteCard insight={engine.insights[0]} phase={phaseKey} phaseLabel={engine.phaseLabel} />
        )}
          ...
          return (
            <div className={cn("glass-card p-4 flex items-start gap-3 border", riskStyle.border)}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-muted-foreground/55 leading-relaxed mt-1">Estimativa informativa — não substitui um método contracetivo nem aconselhamento médico.</p>
              </div>
            </div>
          );
        })()}
        ...
        <div className="glass-card overflow-hidden">
          <SectionHeader accentFrom="from-rose-300/70 via-rose-200/30 dark:from-rose-700/50" markerClass="bg-rose-400/80">
            Menstruação
          </SectionHeader>
          <div className="p-5 space-y-4">
            {openPeriod ? (
              <>
                <div className="rounded-2xl bg-gradient-to-br from-rose-50 to-rose-100/50 dark:from-rose-950/40 dark:to-rose-950/10 border border-rose-100 dark:border-rose-900/50 p-4 flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-rose-500 flex items-center justify-center shrink-0 animate-glow-pulse shadow-[0_0_16px_rgba(244,63,94,0.40)]">
                    <Droplet className="w-4 h-4 text-white" fill="currentColor" strokeWidth={0} />
                  </div>
                  <div className="flex-1 min-w-0">
                  ...
                  <span className="text-xs font-bold bg-white/80 dark:bg-rose-950/50 text-rose-500 border border-rose-200 dark:border-rose-800/60 px-3 py-1 rounded-full tabular-nums shrink-0">{periodDays}d</span>
                </div>
                {!data.isMale && (
                  <div className="space-y-3">
                    <Textarea value={periodNotes} onChange={e => setPeriodNotes(e.target.value)} placeholder="Notas sobre este período (opcional)" disabled={saving} className="rounded-xl border-border bg-card text-sm resize-none min-h-[60px]" />
                    <div className="flex gap-2">
                      <button onClick={() => { setEndDate(today); setShowEndDate(false); handleEndPeriod(); }} disabled={saving}
                        className="flex-1 h-10 flex items-center justify-center gap-1.5 rounded-2xl border border-border/60 text-sm font-medium text-foreground hover:bg-muted transition-all disabled:opacity-60">
                        {saving ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <StopCircle className="h-3.5 w-3.5" strokeWidth={1.5} />}
                        Terminou hoje
                      </button>
                      <button onClick={() => { setShowEndDate(!showEndDate); setEndDate(today); }}
                        className={cn("h-10 px-3 rounded-2xl border text-xs flex items-center gap-1 transition-all duration-200",
                          showEndDate ? "border-rose-300 dark:border-rose-700 bg-rose-50 dark:bg-rose-950/40 text-rose-500" : "border-border/60 text-muted-foreground hover:bg-muted")}>
                        <Calendar className="h-3 w-3" strokeWidth={1.5} /> Outra data
                      </button>
                    </div>
                    {showEndDate && (
                      <div className="space-y-2">
                        <Input type="date" value={endDate} max={today} min={openPeriod?.start_date} onChange={e => setEndDate(e.target.value)} disabled={saving} className="rounded-xl border-border bg-card h-10 text-sm" />
                        <button onClick={handleEndPeriod} disabled={saving || !endDate}
              ...
              <div className="space-y-4">
                ...
                <Textarea value={periodNotes} onChange={e => setPeriodNotes(e.target.value)} placeholder="Notas sobre este período (opcional)" disabled={saving} className="rounded-xl border-border bg-card text-sm resize-none min-h-[60px]" />
                <button onClick={() => { setStartDate(today); setShowBackdate(false); handleStartPeriod(); }} disabled={saving}
                  className="w-full h-12 rounded-2xl bg-rose-500 text-white font-semibold text-sm disabled:opacity-60 active:scale-[0.98] transition-all flex items-center justify-center gap-2 shadow-[0_4px_16px_rgba(244,63,94,0.30)]">
                ...
                <button onClick={() => { setShowBackdate(!showBackdate); setStartDate(today); }} disabled={saving}
                  className={cn("w-full h-10 flex items-center justify-center gap-1.5 rounded-2xl border text-sm font-medium transition-all duration-200 disabled:opacity-60",
                    showBackdate ? "border-rose-300 dark:border-rose-700 bg-rose-50 dark:bg-rose-950/40 text-rose-500" : "border-border/60 text-muted-foreground hover:bg-muted")}>
                  <Calendar className="h-4 w-4" strokeWidth={1.5} /> Começou noutro dia
                </button>
                {showBackdate && (
                  <div className="space-y-2">
                    <Input type="date" value={startDate} max={today} onChange={e => setStartDate(e.target.value)} disabled={saving} className="rounded-xl border-border bg-card h-10 text-sm" />
                    <button onClick={handleStartPeriod} disabled={saving || !startDate}
        ...
        {/* ── Como me sinto hoje — labels com cores próprias ── */}
        <div className="glass-card overflow-hidden">
          <SectionHeader accentFrom="from-violet-300/60 via-fuchsia-200/20 dark:from-violet-700/40" markerClass="bg-violet-400/80">
            Como me sinto hoje
          </SectionHeader>
          <div className="p-5 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3 items-end">
              <div className="space-y-2">
                <p className="text-[11px] font-bold uppercase tracking-widest text-indigo-500 dark:text-indigo-400">Sono (h)</p>
              ...
              <div className="space-y-2">
                <p className="text-[11px] font-bold uppercase tracking-widest text-indigo-500 dark:text-indigo-400">Qualidade do sono</p>
                    ...
                    return (
                      <button key={opt.value} type="button" disabled={data.isMale || saving} onClick={() => setSleepQuality(opt.value)}
                        className={cn("flex-1 h-10 flex items-center justify-center rounded-xl border transition-all duration-200 ease-out",
                          activeQ
                            ? cn(FEELING_STYLES.indigo.border, FEELING_STYLES.indigo.bg, FEELING_STYLES.indigo.text, FEELING_STYLES.indigo.glow, "scale-[1.05]")
                            : "border-indigo-100/70 dark:border-indigo-900/30 bg-indigo-50/40 dark:bg-indigo-950/15 text-indigo-400/60 hover:scale-[1.02] active:scale-95",
                          (data.isMale || saving) && "opacity-50 cursor-not-allowed"
                        )}>
                        <opt.icon className={cn("h-4 w-4 transition-all", activeQ ? "" : "opacity-50")} strokeWidth={activeQ ? 2 : 1.5} />
                      </button>
                    );
                  })}
                </div>
              ...
              <div className="space-y-2">
                <p className="text-[11px] font-bold uppercase tracking-widest text-teal-500 dark:text-teal-400 flex items-center gap-1">
        ...
        <div className="glass-card overflow-hidden">
          <SectionHeader
            accentFrom="from-pink-300/60 via-pink-200/20 dark:from-pink-700/40"
            markerClass="bg-pink-400/80"
            right={
              <button type="button" disabled={data.isMale}
                onClick={() => { if (showIntimacyDate) { setShowIntimacyDate(false); setIntimacyDate(today); } else setShowIntimacyDate(true); }}
                className="h-7 px-3 rounded-full bg-pink-50 dark:bg-pink-950/40 border border-pink-200 dark:border-pink-800/60 text-pink-500 text-[11px] font-bold flex items-center gap-1.5 active:scale-95 transition-all disabled:opacity-50">
          ...
          <div className="p-5 space-y-4">
            {showIntimacyDate && (
              <Input type="date" value={intimacyDate} max={today} onChange={e => setIntimacyDate(e.target.value)} disabled={data.isMale || savingIntimacy} className="rounded-xl border-border bg-card h-10 text-sm" />
            )}

            <div className="space-y-2.5">
              <p className="text-[11px] font-bold uppercase tracking-widest text-pink-500 dark:text-pink-400">Houve relação?</p>
              <div className="flex gap-2">
                {[{ value: false, label: "Não" }, { value: true, label: "Sim" }].map(opt => {
                  const activeYN = hadActivityToday === opt.value;
                  return (
                    <button key={String(opt.value)} type="button" disabled={data.isMale || savingIntimacy} onClick={() => setHadActivityToday(opt.value)}
                      className={cn("flex-1 py-3 rounded-2xl border text-xs font-semibold transition-all duration-200 ease-out",
                        activeYN
                          ? cn(FEELING_STYLES.pink.border, FEELING_STYLES.pink.bg, FEELING_STYLES.pink.text, FEELING_STYLES.pink.glow, "scale-[1.04]")
                          : "border-pink-100/70 dark:border-pink-900/30 bg-pink-50/40 dark:bg-pink-950/15 text-pink-400/60 hover:scale-[1.02] active:scale-95",
            ...
            {hadActivityToday && (
              <StringChipSelector label="Prevenção" options={PROTECTION_OPTIONS} value={protectionMethod} onChange={setProtectionMethod} disabled={data.isMale || savingIntimacy} color="pink" />
            )}

            {!data.isMale && (
              <button onClick={handleSaveIntimacy} disabled={savingIntimacy}
                className="w-full h-11 rounded-2xl bg-pink-500 text-white font-semibold text-sm disabled:opacity-60 active:scale-[0.98] transition-all flex items-center justify-center gap-2 shadow-[0_4px_14px_rgba(236,72,153,0.28)]">
        ...
        {/* ── Sintomas — cada secção com fundo colorido próprio ── */}
        <div className="glass-card overflow-hidden">
          <SectionHeader
            accentFrom="from-rose-300/50 via-violet-200/20 dark:from-rose-700/40"
            markerClass="bg-gradient-to-b from-rose-400/80 to-violet-400/70"
            right={activeSymptoms > 0 ? (
              <span className="px-2.5 py-0.5 rounded-full bg-rose-50 dark:bg-rose-950/40 text-rose-500 border border-rose-200 dark:border-rose-800/60 text-[10px] font-bold">{activeSymptoms} ativos</span>
            ) : undefined}
          >
            Sintomas
          </SectionHeader>
          <div className="p-5 space-y-6">
            {SYMPTOM_SECTIONS.map(section => {
              ...
              return (
                <div key={section.title} className="space-y-3">
                  {/* Título de sub-secção com cor própria */}
                  <div className="flex items-center gap-1.5">
                    <div className={cn("w-1 h-3 rounded-full shrink-0", style.solid)} />
                    <p className={cn("text-[11px] font-bold uppercase tracking-widest", style.labelColor)}>{section.title}</p>
                  </div>
                    ...
                    {section.items.map(s => {
                      ...
                      return (
                        <button key={s.key} type="button" disabled={data.isMale}
                          ...
                          className={cn(
                            "flex flex-col items-center gap-1.5 rounded-2xl border p-2.5 text-[10px] font-semibold transition-all duration-200 ease-out",
                            active
                              ? cn(style.border, style.soft, style.glow, "scale-[1.06]")
                              : cn(style.inactiveBorder, style.inactiveBg, "hover:scale-[1.02] active:scale-95"),
                          ...
                          <div className={cn("w-9 h-9 rounded-full flex items-center justify-center transition-all duration-200",
                            ...
                            <Icon className={cn("h-[17px] w-[17px] transition-all duration-200",
                              active ? "text-white" : cn(style.text, "opacity-75"))} strokeWidth={active ? 2 : 1.75} />
                          </div>
                          <span className={cn("leading-tight text-center transition-colors duration-200",
                            active ? style.text : cn(style.text, "opacity-60"))}>
                            {s.label}
                          </span>
                        </button>
                      );
            ...
            <div className="space-y-2">
              ...
              <Textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Algo que queiras registar sobre hoje..." disabled={data.isMale || saving} className="rounded-xl border-border bg-card text-sm resize-none min-h-[80px]" />
            </div>

            {!data.isMale && (
              <button onClick={handleSaveSymptoms} disabled={saving}
                className="w-full h-12 rounded-2xl bg-rose-500 text-white font-semibold text-sm disabled:opacity-60 active:scale-[0.98] transition-all flex items-center justify-center gap-2 shadow-[0_4px_16px_rgba(244,63,94,0.28)]">
      ...
      {/* ── Toast de carinho — desliza suavemente e desaparece ── */}
      <div className={cn(
        ...
        <div className="bg-card/95 backdrop-blur-sm border border-border/50 shadow-xl rounded-2xl px-5 py-3.5 flex items-center gap-3">
          <div className="w-7 h-7 rounded-full bg-rose-50 dark:bg-rose-950/50 border border-rose-200/60 dark:border-rose-800/40 flex items-center justify-center shrink-0">
