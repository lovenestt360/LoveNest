...
const DEFAULT_STATE: GuardianState = { glowColor: "rose", ringUnlocked: false };
  ...
  const fetchState = useCallback(async () => {
    ...
    try {
      const [{ data: guardian }, { data: purchases }] = await Promise.all([
        supabase.from("guardian_state" as any).select("glow_color, ring_unlocked").eq("couple_space_id", coupleSpaceId).maybeSingle(),
