...
BEGIN
    -- Upsert points
    INSERT INTO public.love_points (user_id, couple_id, points, updated_at)
    VALUES (p_user_id, p_couple_id, p_amount, now())
    ...
    -- Log history
    INSERT INTO public.love_points_history (user_id, couple_id, amount, reason)
    VALUES (p_user_id, p_couple_id, p_amount, p_reason);
END;
...
BEGIN
    ...
    -- Award points
    PERFORM public.fn_award_points(NEW.user_id, NEW.couple_id, COALESCE(v_reward, 10), 'mission_completed');
...
BEGIN
    ...
    IF NEW.current_streak > OLD.current_streak THEN
        ...
        FOR v_member IN 
            SELECT user_id FROM public.members WHERE couple_space_id = NEW.couple_id
        LOOP
            -- Award 50 points to each for maintaining the connection
            PERFORM public.fn_award_points(v_member.user_id, NEW.couple_id, 50, 'streak_maintained');
        END LOOP;
    END IF;
...
DROP TRIGGER IF EXISTS tr_streak_increment_points_trigger ON public.love_streak;
CREATE TRIGGER tr_streak_increment_points_trigger
AFTER UPDATE ON public.love_streak
...
-- This assumes some shared points might be in a 'total_points' column of 'love_streak' or 'love_streaks'
DO $$
DECLARE
    v_streak RECORD;
    v_member RECORD;
BEGIN
    -- Assuming 'love_streak' might have a 'total_points' column we can migrate from
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='love_streak' AND column_name='total_points') THEN
        FOR v_streak IN SELECT couple_id, total_points FROM public.love_streak WHERE total_points > 0 LOOP
            FOR v_member IN SELECT user_id FROM public.members WHERE couple_space_id = v_streak.couple_id LOOP
                PERFORM public.fn_award_points(v_member.user_id, v_streak.couple_id, v_streak.total_points, 'shared_points_migration');
            END LOOP;
        END LOOP;
    END IF;
END $$;
