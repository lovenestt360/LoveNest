-- ============================================================
-- CORREÇÃO DA CONTAGEM E SINCRONIZAÇÃO DE MISSÕES (V9)
-- Resolvendo erro de "5 para 4" no progresso.
-- ============================================================

-- 1. Melhorar a RPC de geração e consulta para ser ultra-precisa no progresso
...
BEGIN
    -- Gerar 3 missões para hoje se ainda não existirem (Consistência)
    IF NOT EXISTS (
    ...
    -- Retornar missões com progresso GARANTIDO
    RETURN QUERY
    SELECT
        cdm.id                              AS cdm_id,
        cdm.mission_id,
...
-- 2. Refinar checkMissionCompletion para ser invocado pelo motor V8 com segurança
...
BEGIN
    ...
    LOOP
        ...
        WHERE user_id = p_user_id
          ...
          -- Usar o mesmo filtro de data da RPC
